...
export default class TheEntityListContent extends Vue {
  ...
  get entities (): Entity[] {
    const apiData: Api | undefined = this.api.find((api: Api) => api.name === GameApiRoutes[Game.WOT].entities)
    if (apiData?.data) {
      const entities = (apiData.data as EntitiesDataApi).entities as Tank[]
      return entities.filter((tank: Tank) => !tank.default).map((tank: Tank): Tank => ({
        id: tank.id,
        apiId: tank.apiId,
        name: tank.name,
        title: tank.title,
        ...
        image: tank.image,
        type: tank.type,
        default: tank.default,
        canvasImage: tank.canvasImage,
        color: tank.color
      }))
    }
    return []
  ...
  entityChipGroupOnChangeHandler (entity: Tank) {
